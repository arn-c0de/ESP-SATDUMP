...
static const int16_t LEFT_MARGIN  = 32;   // space for PRN label
static const int16_t RIGHT_MARGIN = 30;   // space for SNR value
static const int16_t BAR_MAX_W    = TFT_WIDTH - LEFT_MARGIN - RIGHT_MARGIN - 4;
static const int16_t ROW_H        = (TFT_HEIGHT - STATUS_BAR_H - 4) / MAX_SATS;
static const int16_t BAR_H        = ROW_H - 3;
...
static void drawBars() {
    ...
    for (uint8_t r = 0; r < MAX_SATS; r++) {
        ...
        tft.fillRect(0, ry, TFT_WIDTH, ROW_H, COL_BG);
    ...
    if (!gpsData.fix_quality) {
        ...
        tft.setTextSize(2);
        tft.setCursor((TFT_WIDTH - 12*12) / 2, TFT_HEIGHT / 2 - 8);
        tft.print("NO FIX");
    }
}
