...
try:
    ...
    # Many ESP32 dev boards wire RTS/DTR to EN/GPIO0 for auto-reset/bootloader.
    # PySerial may assert these on open, which can hold the MCU in reset/boot mode.
    ser.dtr = False
    ser.rts = False
...
try:
    while True:
        ...
        if select.select([sys.stdin], [], [], 0.01)[0]:
            ...
            # ESP32 sketch expects '\n' as line terminator and ignores '\r'.
            # In raw TTY mode, Enter is typically '\r', so translate it to '\n'
            # (and echo as CRLF for a clean local display).
            if char in (b'\r', b'\n'):
